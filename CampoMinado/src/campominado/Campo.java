		...
		for (int i = 0;	 i < Constantes.NUMERO_LINHAS; i++) {
			for (int j = 0;	 j < Constantes.NUMERO_LINHAS; j++) {
				if (i > 0) {
					...
					if (j > Constantes.NUMERO_COLUNAS-1) matriz[i][j].adicionarVizinhos(matriz[i-1][j+1]);
				}
				
				if (j > 0) matriz[i][j].adicionarVizinhos(matriz[i][j-1]);
				if (j > Constantes.NUMERO_COLUNAS-1)matriz[i][j].adicionarVizinhos(matriz[i][j+1]);
